...
if ! curl -fsSL --retry 3 --retry-delay 3 -m 180 -o ${PREFLIGHT_DIR}/jq https://github.com/jqlang/jq/releases/latest/download/jq-linux-${JQ_ARCH} \
...
if ! curl -fsSL --retry 3 --retry-delay 3 -m 300 -o ${PREFLIGHT_DIR}/librespot https://github.com/friebi/MuPiBox/raw/refs/heads/main/bin/librespot/dev_0.6_20250806/librespot-${LIBRESPOT_ARCH} \
...
if [ -n "$BRANCH" ]; then
  if [ -n "$VERSION_LABEL" ]; then
	...
	VERSION_LONG="DEV ${VERSION} $(curl -s "https://api.github.com/repos/friebi/MuPiBox" | jq -r '.pushed_at' | cut -d'T' -f1)"  >&3 2>&3
else
