...
export class PlayerPage implements OnInit {
    ...
    if (this.media.type === "spotify") {
      ...
      if (this.media.showid?.length > 0) {
        ...
        if (this.currentPlayedSpotify?.item != null) {
          this.progress =
            (seek / this.currentPlayedSpotify?.item.duration_ms) * 100 || 0;
        }
      }
      ...
      if (
        ...
        this.currentPlayedLocal?.currentTracknr ===
          this.currentPlayedLocal?.totalTracks
      ) {
        this.goBackTimer++;
        if (this.goBackTimer > 10) {
          this.navController.back();
  ...
  ionViewWillLeave() {
    ...
    if (
      this.media.type === "spotify" &&
      (this.media.category === "music" || this.media.category === "other")
    ) {
      ...
      for (let i = 1; i < this.media.resumelocalcurrentTracknr; i++) {
        setTimeout(() => {
          ...
          if (j === this.media.resumelocalcurrentTracknr) {
            setTimeout(() => {
              this.playerService.seekPosition(
                this.media.resumelocalprogressTime
              );
            }, 2000);
          }
        }, 2000);
  ...
  saveResumeFiles() {
    ...
    if (this.resumemedia.type === "spotify" && this.resumemedia?.showid) {
      ...
      this.resumemedia.resumespotifyprogress_ms =
        this.currentPlayedSpotify?.progress_ms || 0;
      this.resumemedia.resumespotifyduration_ms =
        this.currentEpisode?.duration_ms || 0;
      ...
      if (this.resumemedia.playlistid) {
        ...
        this.resumemedia.resumespotifytrack_number =
          this.currentPlayedSpotify?.item.track_number || 0;
      }
      this.resumemedia.resumespotifyprogress_ms =
        this.currentPlayedSpotify?.progress_ms || 0;
      this.resumemedia.resumespotifyduration_ms =
        this.currentPlayedSpotify?.item.duration_ms || 0;
    } else if (this.resumemedia.type === "library") {
      this.resumemedia.resumelocalalbum = this.resumemedia.category;
      this.resumemedia.resumelocalcurrentTracknr =
        this.currentPlayedLocal?.currentTracknr || 0;
      this.resumemedia.resumelocalprogressTime =
        this.currentPlayedLocal?.progressTime || 0;
    } else if (this.resumemedia.type === "rss") {
      this.resumemedia.resumerssprogressTime =
        this.currentPlayedLocal?.progressTime || 0;
    }
    ...
    if (this.resumePlay || this.resumeIndex !== -1 || this.resumeAdded) {
      ...
      this.mediaService.editRawResumeAtIndex(
        this.resumeIndex,
        this.resumemedia
      );
