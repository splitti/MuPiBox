...
import { BehaviorSubject, catchError, EMPTY, firstValueFrom, from, interval, Observable, of } from 'rxjs'
...
export class SpotifyService {
  ...
  private async testPlayerFunctionality(): Promise<boolean> {
    ...
    try {
      const _state = await Promise.race([
  ...
  private async initializeWebPlaybackSDKSync(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      const _originalLoadingPromise = this.sdkLoadingPromise
