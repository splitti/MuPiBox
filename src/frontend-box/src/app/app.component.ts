...
import { catchError, distinctUntilChanged, interval, map, Observable, of, switchMap } from 'rxjs'
        ...
        switchMap(
          (): Observable<Monitor> =>
            this.http.get<Monitor>(`${environment.backend.apiUrl}/monitor`).pipe(
              // A failed poll must not end the stream - the screen would then be stuck
              // in whatever state it had when the backend hiccuped.
              catchError(() => of({ monitor: 'On' } as Monitor)),
            ),
        ),
