			...
			<ul>
				...
				<p><h2>MuPiBox-News</h2>
					<?php
						...
						$news_url = 'https://raw.githubusercontent.com/friebi/MuPiBox/main/news.txt';
						$news_text = @file_get_contents($news_url, false, stream_context_create(['http' => ['timeout' => 8]]));
						if ($news_text === false || trim($news_text) === '') {
							print 'The news could not be loaded (no connection to GitHub). See <a href="https://github.com/friebi/MuPiBox/blob/main/news.txt" target="_blank">news.txt</a>.';
						} else {
							print $news_text;
						}
					?>
